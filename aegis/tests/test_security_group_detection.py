...
def make_event(rule: NetworkRule) -> NormalizedEvent:
...
def test_detects_public_ipv4_ssh():
    ...
    assert detections[0].rule_id == "AEGIS-AWS-SG-001"
    assert detections[0].severity == Severity.HIGH


def test_detects_port_range_containing_ssh():
    event = make_event(
        NetworkRule(
            protocol="tcp",
            from_port=1,
...
def test_detects_all_traffic_from_internet():
    event = make_event(
        NetworkRule(
            protocol="-1",
            from_port=None,
...
def test_ignores_unrelated_event_action():
    ...
    event.action = "RevokeSecurityGroupIngress"
